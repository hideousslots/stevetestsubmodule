...
export class SimpleVisualReels extends PIXIContainerBase {
	...
	protected symbolGrid: PIXI.Text[][] = [];
	protected passedConfig: any;
	...
	constructor(config: any) {
		...
		for (let reel = 0; reel < this.passedConfig.reelCount; reel++) {
			...
			for (let row = 0; row < this.passedConfig.rowCount; row++) {
				const newText = new PIXI.Text(
					this.passedConfig.symbolMap[
						Math.floor(
							Math.random() * this.passedConfig.symbolMap.length,
						)
					],
					{
						fontFamily: 'Arial',
		...
		const bounds: PIXI.Rectangle = this.getBounds();
